...
if (isset($_GET['code'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      $sql = "INSERT INTO users (user_email, user_gender, user_fullname, user_profileimg, email_verified, user_token,user_username,user_ipaddress,user_role,user_createdat) VALUES ('{$userinfo['user_email']}', '{$userinfo['gender']}', '{$userinfo['user_fullname']}', '{$userinfo['user_profileimg']}', '{$userinfo['verifiedEmail']}', '{$userinfo['token']}','{$user_username}','{$client_ip}','user',CURRENT_TIMESTAMP())";
