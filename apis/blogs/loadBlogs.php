...
if($category != "any"){
    $loadBlogQuery = "SELECT *,DATE_FORMAT(last_updated,'%d %b, %Y') as last_updated FROM `blogs` b INNER JOIN `users` u ON b.blog_publisher=u.user_id WHERE b.blog_status='active' AND b.blog_category={$category} ORDER BY blog_id DESC LIMIT $start,2";
}else{
